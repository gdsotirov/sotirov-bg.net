...
<xsl:template match="/">
  ...
  <body>
    <xsl:attribute name="style">font-family: sans-serif; font-size: normal; background-color: #ffffff;</xsl:attribute>
...
Заглавие: <xsl:value-of select="title"/><br/>
Обновена на: <xsl:value-of select="lastBuildDate"/><br/>
<xsl:choose>
<xsl:when test="count(item) = 1">
Показанa е само една публикация в тази емисия.
</xsl:when>
</xsl:choose>
</p>
...
Преглеждате RSS емисия. Тя е представена като HTML с използването на XSL стилови описания. <br />
За да видите скрития XML код, моля изберете "Изходен код" командата в браузъра ви.
</i>
</p>
<hr />
<table>
...
<xsl:attribute name="style">font-size: normal;</xsl:attribute>
<xsl:for-each select="item">
  ...
  (Публикувано: <xsl:value-of select="pubDate"/>)
  </td>
